	...
	switch ($action)
	{
		...
		case "chargeListeEtudiants":
			...
			while($user = mysql_fetch_array($res))
			{
				...
				if($user["niveau"]==-1)$user["statut"]=0;
			...
			$req = "SELECT cycles.nom, cycles.id FROM cycles ORDER BY cycles.semestre_debut ASC;";
			$res = mysql_query($req);
			$cycles = array();
			$cycles["text"] = array();
			$cycles["value"] = array();
			...
			break;
		case "chargeCycles" :
			$req = "SELECT cycles.id, cycles.nom FROM cycles WHERE cycles.ecole='$ecole' ORDER BY cycles.semestre_debut";
			$res = mysql_query($req);
			$cycles = array();
			$cycles["text"] = array();
			$cycles["value"] = array();
		...
		case "changeCycleEtudiant":
			...
			echo "var elt=gEBI('niveau_$id');elt.clearOptions();elt.appendOption('-',-1);";
		...
		case "changeNiveauEtudiant":
			...
			$res = mysql_query($req);
			if($niveau==-1){echo "changeCycle();";break;}
