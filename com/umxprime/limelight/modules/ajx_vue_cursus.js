...
function init()
{
	...
	AJX.send("init");
	addListener(gEBI("etudiant"),"change",changeEtudiant);
	gEBI("etudiant").value = gVBI("etudiant_id");
}

addListener(window,"load",init);

function changeEtudiant()
{
	var semestre_courant = gVBI("semestre_courant");
	var etudiant = gVBI("etudiant");
	window.location = "?nPeriode="+semestre_courant+"&id="+etudiant;
}