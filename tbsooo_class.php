...
class clsTinyButStrongOOo extends clsTinyButStrong
{
  ...
  function LoadXmlFromDoc($xml_file)
  {
	...
	exec("chmod -R a+rw ".$this->_ooo_basename);
	/*
	$zip = new ZipArchive;
	if ($zip->open($this->_ooo_basename.'.'.$this->_ooo_file_ext) === TRUE) {
	    $zip->extractTo($this->_ooo_basename,array('content.xml'));
	    $zip->close();
		exec("chmod -R a+rw ".$this->_ooo_basename);
	    //echo 'ok';
	} else {
  ...
  function SaveXmlToDoc()
  {
	...
	$zip = new ZipArchive;
	if ($zip->open($this->_ooo_basename.'.'.$this->_ooo_file_ext) === TRUE) {
		$zip->deleteName($this->_xml_filename);
	    $zip->addFile($this->_ooo_basename.'/'.$this->_xml_filename,$this->_xml_filename);
	    $zip->close();
	} else {
	    echo 'failed';
	}
